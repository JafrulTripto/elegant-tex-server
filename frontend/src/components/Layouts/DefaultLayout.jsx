...
const DefaultLayout = () => {
  ...
  const fetchUser = useCallback(() => {
    axiosClient
      ...
      .then(({ data }) => {
        ...
        setRoles(data.roles);
        setUnseenReleases(data.unseen_releases ?? []);
      })
      .catch((error) => {
  ...
  }, [axiosClient, setRoles, setPermissions, setUser, setUnseenReleases]);
  ...
  return (
    <Layout className="h-screen overflow-hidden">
      ...
      >
        ...
        >
          <div
            className={`transition-all duration-300 flex items-center gap-3 ${collapsed ? "scale-0 w-0 opacity-0 overflow-hidden" : "scale-100 opacity-100"}`}
          >
            {/* Logo Image */}
            <img
              src="/eleganttexlogo.png"
              alt="Elegant Tex"
            ...
            {/* Brand Name */}
            <span
              className={`font-extrabold text-xl whitespace-nowrap tracking-widest ${darkMode ? "text-white" : "text-slate-800"}`}
            >
              ELEGANT TEX
            </span>
          </div>
      ...
      <Layout className="h-full overflow-hidden relative flex flex-col">
        ...
        <Footer className="text-center bg-transparent p-1">
          <Typography.Text type="secondary">
            Elegant Tex Server ©{new Date().getFullYear()} Created by
            Tripzin.inc
          </Typography.Text>
        </Footer>
      </Layout>
